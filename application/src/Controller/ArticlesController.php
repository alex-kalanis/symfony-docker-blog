...
class ArticlesController extends AAppController
{
    /**
    ...
    public function listing(Request $request)
    {
        ...
        $search = new Search(new Libs\Mappers\ArticleRecord());
        if (!$showDeleted) {
            $search->null('deleted');
        }
        $table = new Libs\Tables\ArticleTable($this->inputVariables());
    ...
    public function preview(Request $request)
    {
        ...
        $toPre = strval($request->request->get('preview', ''));

        return $this->render('admin/articles/preview.html.twig', [
            'controller_name' => 'ArticlesController',
            'content' => $markdown->transform($toPre),
        ]);
    }
    ...
    {
        $record = new Libs\Mappers\ArticleRecord();
        $record->id = $id;

        if (0 < $record->count()) {
            $record->load();
            $record->publish = date('Y-m-d H:i:s');
            $record->save();
        }
