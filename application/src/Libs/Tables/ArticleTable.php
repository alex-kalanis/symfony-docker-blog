 ...
 * Complete articles table description
 */
class ArticleTable
{
    /** @var IFiltered */
    ...
    public function composeWeb(Search $search): void
    {
        ...
        $this->table->addOrderedColumn('Title', new Columns\Bold('title'), new KwField\TextContains());
        $this->table->addOrderedColumn('Publish', new Columns\Date('publish', 'd.m.Y', false));
        $this->table->addOrderedColumn('Content', new Columns\Func('id', [$this, 'contentLink']), new KwField\TextContains());

        $columnActions = new Columns\Multi('&nbsp;&nbsp;', 'id');
        $columnActions->addColumn(new Columns\Func('id', [$this, 'editLink']));
        $columnActions->addColumn(new Columns\Func('id', [$this, 'approveLink']));
    ...
    {
        $this->forward->setLink('/admin/article-edit/' . $id);
        $this->forward->setForward('/admin/articles');
    ...
    public function approveLink($id): string
    {
        $this->forward->setLink('/admin/article-approve/' . $id);
        $this->forward->setForward('/admin/articles');
        return sprintf('<a href="%s" title="%s" class="button button-check"> &#x1F589; </a>',
            $this->forward->getLink(),
            'Update'
        );
    }
    ...
    {
        $this->forward->setLink('/admin/article-remove/' . $id);
        $this->forward->setForward('/admin/articles');
