 ...
 * Complete users table description
 */
class UserTable
{
    /** @var IFiltered */
    ...
    public function composeWeb(Search $search): void
    {
        ...
        $this->table->addOrderedColumn('Login', new Columns\Bold('login'), new KwField\TextContains());
        $this->table->addOrderedColumn('Display', new Columns\Basic('display'), new KwField\TextContains());
    ...
    {
        $this->forward->setLink('/admin/user-edit/' . $id);
    ...
    {
        $this->forward->setLink('/admin/user-remove/' . $id);
        $this->forward->setForward('/admin/users');
